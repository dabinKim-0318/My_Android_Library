...
/*    이전 어댑터와 바뀌는 어댑터의 아이템이 동일한 지 확인을 합니다.
    각 아이템의 고유 ID값(DB에서 key 같은 것을 받아온다면 그걸 활용)이 있을 것입니다.
    이를 활용하여 비교를 해주시면 됩니다.
    만약 여기서 areItemsTheSame 를 잘못 정의하시면 모든 Item을 다 지우고
    새 아이템을 Insert하는 일이 초래되어서 notifyDataSetChanged()와 별반 다를 바 없어질 수가 있습니다.*/
